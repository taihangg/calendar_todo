...
//  #region C#: 从 Javascript 代码中提取出来的其他字段(属性)
  ...
  void getLunarFestival() {
    // 按农历日期查找重点节假日
    if (this.lunarRunyue != "闰") {
      //闰月不算节日
      var str = "${this.lunarMonthIndex + 1}-${this.lunarDayIndex + 1}";
      this.lunarFestival = obb.LunarFeasts[str] ?? "";
    }
  }
}
...
class LunarMonth {
  ...
  void yueLiCalc(int By, int Bm, [int dayCount]) {
    ...
    for (int i = 0, j = 0; i < monthDaysCount; i++) {
      ...
      day.lunarDayIndex = (day.d0 - SSQ.HS[mk]).toInt(); // 距农历月首的编移量,0对应初一
      day.lunarDayName = obb.rmc[day.lunarDayIndex]; // 农历日名称
