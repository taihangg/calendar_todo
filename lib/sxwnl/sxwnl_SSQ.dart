...
//  #region 私有字段
  ...
  /// 朔修正表
  /// </summary>
  static String SB = SSQ.init(1); // 朔修正表

  /// <summary>
  /// 气修正表
  /// </summary>
  static String QB = SSQ.init(0); // 气修正表

  /// <summary>
  /// 朔直线拟合参数
  /// </summary>
  static List<double> suoKB = [
    //朔直线拟合参数
  ...
  static List<double> qiKB = [
    ...
    2188621.191481,
    15.218437484, // 1280-02-13 h=0.00013 元史·历志(郭守敬授时历) 回归年=365.24250
...
//  #region 私有方法
  ...
  static double qi_high(double W) {
    ...
    if (v < 600 || v > 86400 - 600)
      t = XL.S_aLon_t(W) * 36525 - JD.deltatT2(t) + 8 / 24;
    return t;
  }
  ...
  static double so_high(double W) {
    ...
    double v = ((t + 0.5) % 1) * 86400;
    if (v < 600 || v > 86400 - 600)
      t = XL.MS_aLon_t(W) * 36525 - JD.deltatT2(t) + 8 / 24;
    return t;
  }
  ...
  /// <param name="s"></param>
  /// <returns></returns>
  static String jieya(String s) {
    String o = "0000000000", o2 = o + o;
    StringBuffer sb = new StringBuffer.withCapacity(s, 32768);
    sb.Replace("J", "00");
    sb.Replace("I", "000");
    sb.Replace("H", "0000");
    sb.Replace("G", "00000");
  ...
  /// <returns></returns>
  static String init(int type) {
    StringBuffer suoS = new StringBuffer(), qiS = new StringBuffer();
  ...
  static double calc(double jd, String qs) {
    jd += LunarHelper.J2000; // C#: 改为常量
    int i;
    double D;
    String n;
    List<double> B = SSQ.suoKB;
    double pc = 14;
    ...
    if (jd < f1 || jd >= f3) {
      ...
      if (qs == "气")
        return Math.Floor2Double(SSQ.qi_high(
                Math.Floor2Double((jd + pc - 2451259) / 365.2422 * 24) *
                    Math.PI /
                    12) +
            0.5); //2451259是1999.3.21,太阳视黄经为0,春分.定气计算
      else
        return Math.Floor2Double(SSQ.so_high(
                Math.Floor2Double((jd + pc - 2451551) / 29.5306) *
                    Math.PI *
                    2) +
    ...
    if (jd >= f1 && jd < f2) {
      ...
      D = B[i] + B[i + 1] * Math.Floor2Double((jd + pc - B[i]) / B[i + 1]);
    ...
    if (jd >= f2 && jd < f3) {
      // 定气或定朔
      if (qs == "气") {
        D = Math.Floor2Double(SSQ.qi_low(
                Math.Floor2Double((jd + pc - 2451259) / 365.2422 * 24) *
                    Math.PI /
                    12) +
            0.5); //2451259是1999.3.21,太阳视黄经为0,春分.定气计算
        var t = (Math.Floor2Double((jd - f2) / 365.2422 * 24)).toInt();
        n = SSQ.QB.substring(t, t + 1); //找定气修正值
      } else {
        D = Math.Floor2Double(SSQ.so_low(
                Math.Floor2Double((jd + pc - 2451551) / 29.5306) *
                    Math.PI *
                    2) +
            0.5); //2451551是2000.1.7的那个朔日,黄经差为0.定朔计算
        var t = (Math.Floor2Double((jd - f2) / 29.5306)).toInt();
        n = SSQ.SB.substring(t, t + 1); //找定朔修正值
      }
      if (n == "1") return D + 1;
      if (n == "2") return D - 1;
      return D;
...
//  #region 公共属性(注: 初始转换时为公共字段, 已改写, 请参阅“转换时增加的私有字段”)
  ...
  static void calcY(double jd) {
    ...
    List<double> B = SSQ.HS; // 中气表,日月合朔表(整日)
    int i;
    double W, w;

    // 该年的气
    W = Math.Floor2Double((jd - 355 + 183) / 365.2422) * 365.2422 +
        355; // 355是2000.12冬至,得到较靠近jd的冬至估计值
    if (SSQ.calc(W, "气") > jd) W -= 365.2422;
    for (i = 0; i < 25; i++)
      A[i] = SSQ.calc(W + 15.2184 * i, "气"); // 25个节气时刻(北京时间),从冬至开始到下一个冬至以后
    A.pe1 = SSQ.calc(W - 15.2, "气");
    A.pe2 = SSQ.calc(W - 30.4, "气"); // 补算二气,确保一年中所有月份的“气”全部被计算在内
    ...
    double YY =
        Math.Floor2Double((SSQ.ZQ[0] + 10 + 180) / 365.2422) + 2000; // 确定年份
    if (YY >= -721 && YY <= -104) {
      double ly = 0,
          b0 = 0,
          k0 = 0,
          x0 = 0; // ly为历元(本历首月的儒略数),x0月建,lName闰月名称,b0,k0为置闰拟合参数
      String lName = "";
      if (YY >= -721) {
        ly = 1457698 - LunarHelper.J2000;
        k0 = 12.368422;
        b0 = 0.342;
        lName = "十三";
        x0 = 2;
      ...
      if (YY >= -220) {
        ly = 1640641 - LunarHelper.J2000;
        k0 = 12.369000;
        b0 = 0.866;
        lName = "后九";
        x0 = 11;
      } // 秦汉历,ly为-221.10.31;
      double nY = Math.Floor2Double((SSQ.HS[0] - ly + 100) / 365.25); // 年积数
      double f1 = Math.Floor2Double(b0 + nY * k0),
          f2 = Math.Floor2Double(b0 + nY * k0 + k0),
          f3; // f1有本年首的月积数,f2为下一年首的月积数
      f1 = Math.Floor2Double(f1);
      f2 = Math.Floor2Double(f2);
      for (i = 0; i < 14; i++) {
        f3 = Math.Floor2Double((SSQ.HS[i] - ly + 15) / 29.5306); // 该月积数
        if (f3 < f2)
          f3 -= f1;
        else
          f3 -= f2;
        if (f3 < 12)
          SSQ.ym[i] = obb.ymc[((f3 + x0) % 12).toInt()];
        else
          SSQ.ym[i] = lName;
      }
    }
    ...
    // 名称转换(月建别名)
    for (i = 0; i < 14; i++) {
      double Dm = SSQ.HS[i] + LunarHelper.J2000,
          v2 = int.parse(SSQ.ym[i]).toDouble(); // Dm初一的儒略日,v2为月建序号
      String mc = obb.ymc[(v2 % 12).toInt()]; // 月建对应的默认月名称：建子十一,建丑十二,建寅为正……
      if (Dm >= 1724360 && Dm <= 1729794)
        mc = obb.ymc[((v2 + 1) % 12).toInt()]; //  8.01.15至 23.12.02 建子为十二,其它顺推
      else if (Dm >= 1807724 && Dm <= 1808699)
        mc = obb.ymc[((v2 + 1) % 12).toInt()]; // 237.04.12至239.12.13 建子为十二,其它顺推
      else if (Dm >= 1999349 && Dm <= 1999467)
        mc = obb.ymc[((v2 + 2) % 12).toInt()]; // 761.12.02至762.03.30 建子为正月,其它顺推
...
//  #region 转换时增加的私有字段(用于封装成公共属性, 按转换规范 10 命名)
  ...
  static List<String> ym = List(32); //各月名称
  static LunarInfoListT<double> ZQ = new LunarInfoListT<double>.withData(
      31, 0); //中气表,其中.liqiu是节气立秋的儒略日,计算三伏时用到
  static List<double> HS = List(32); //合朔表
  static List<double> dx = List(32); //各月大小
  static List<double> Yn = List(1); //年计数
...
//  #region 转换时新增加的方法
  ...
  /// <returns></returns>
  static int calcJieQi(double jd, bool calcMultiPeriod) {
    LunarInfoListT<double> A = SSQ.ZQ;
    List<double> B = SSQ.HS; // 中气表,日月合朔表(整日)
    int i;
    double W;
    ...
    {
      W = Math.Floor2Double((jd - 355 + 183) / 365.2422) * 365.2422 +
          296; // 296是2000.10.23霜降(距200.1.1的天数),得到较靠近jd的霜降估计值
      if (SSQ.calc(W, "气") > jd) W -= 365.2422;
      for (i = 0; i < 31; i++)
